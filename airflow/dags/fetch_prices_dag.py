...
def task_fetch_prices():
    ...
    try:
        logger.info(f"Fetching prices for {json_data['origin']} -> {json_data['destination']}")
        response = requests.post(f"http://{HOST}:8000/fetch-prices", json=json_data, timeout=30)
